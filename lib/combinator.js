...
Combinator.prototype.compressJs = function(cacheKey, liveOrLatest, res, callback) {
	
	var self = this;

	var liveCacheKey = cacheKey + liveOrLatest;

	if(cacheKey && self.cache[liveCacheKey] && liveOrLatest == 'live') {
		res.header('X-Repo-Cached',cacheKey + liveOrLatest);
		return callback(null, self.cache[liveCacheKey]);
	}
...
Combinator.prototype.compressCss = function(cacheKey, liveOrLatest, res, callback) {
		
	var self = this;
	
	liveCacheKey = cacheKey + liveOrLatest;
